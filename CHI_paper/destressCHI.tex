...
Our goal is to perform semantic searches on the dataset from queries which can range from individual words to full sentences. To do this, we require a procedure for mapping sentences to the word vector space so that the cosine similarity still makes sense. We do this by first normalizing the rows of the embedding matrix, so that each word in the dictionary has the same norm. A sentence is then embedded by summing the projections of each constituent word. The question is posed to the data set in this embedding. Note that multi-sentence queries are also possible.
