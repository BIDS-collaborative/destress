...
\caption{Some example queries and results on the full data set using our method.  \label{fig:reminiscenceTriggers}}
\end{figure*}
...
In this paper, we presented a new way to explore a text corpus by taking advantage of semantic embeddings, designed with the feedback of several qualitative researchers. We showed that this can be a useful addition to a qualitative researcher's toolbox. 

In the future, we plan to implement to a distributed system to query in real-time on the full dataset and to develop a better user interface for performing queries.
